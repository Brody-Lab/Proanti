...
dp=2*qfuncinv(.05+1-classifier_results);
%%% d' to classification:
% % y=1-qfunc(dp/2);

figure; plot(1:105,dp)


vals=max(dp*.75);


latency=nan(size(dp,1),1);
for i=1:size(dp,1)
    vec=dp(i,:);
    vec=smooth(vec)';
...
classifier_results=res_choice_sc_full;
dp=2*qfuncinv(.05+1-classifier_results);
%%% d' to classification:
% % y=1-qfunc(dp/2);

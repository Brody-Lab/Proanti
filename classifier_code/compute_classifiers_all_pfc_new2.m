...
res_ap_pfc=nan(n_iterations,length(centers),length(neurvals));
res_choice_pfc=nan(n_iterations,length(centers),length(neurvals));
...
for jjj=1:length(neurvals)
    ...
    for i=1:n_iterations
        ...
        for tx=1:length(centers) %look over different time points
            ...
            res_ap_pfc(i,tx,jjj)=mean([valp vala]);
            ...
            res_choice_pfc(i,tx,jjj)=mean([valp vala]);
...
save ../data_files/svm_results_pfc_new2 res_ap_pfc ...
    res_choice_pfc centers neurvals

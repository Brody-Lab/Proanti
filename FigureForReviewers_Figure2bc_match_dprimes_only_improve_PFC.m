...
load data_files/svm_results_pfc_most_informative_29
res_choice_pfc_most_info=res_choice_pfc;
...
plot(centers,mean(res_choice_pfc_most_info),'k:','LineWidth',3)
...
y = interp1(centers,mean(res_choice_pfc_most_info),x);
[~,ind]=min(abs(y-0.65));
plot(x(ind),y(ind),'k.','MarkerSize',50);
...
legend('SC all','PFC all','best PFC cells');

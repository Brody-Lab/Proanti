...
for ccx=1:numel(CD.cellid)
    ...
    for j=1:length(centers)
        matsc_err_ap{z,j,1}=qy(pro_err,j);
        matsc_err_ap{z,j,2}=qy(anti_err,j);
    end
    
    
    for j=1:length(centers)
        matsc_err_choice{z,j,1}=qy(contra_err,j);
        matsc_err_choice{z,j,2}=qy(ipsi_err,j);
    end
...
save ../data_files/matsc matsc matsc_err_ap ...
    matsc_err_choice waves_sc centers bin_size

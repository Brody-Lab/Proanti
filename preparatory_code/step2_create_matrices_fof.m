...
for ccx=1:numel(CD.cellid)
    ...
    matfof(z,:,:,1,1)=qy(proc,:)';
    matfof(z,:,:,1,2)=qy(proi,:)';
    matfof(z,:,:,2,1)=qy(antic,:)';
    matfof(z,:,:,2,2)=qy(antii,:)';
    
    
    for j=1:length(centers)
        matfof_err_ap{z,j,1}=qy(pro_err,j);
        matfof_err_ap{z,j,2}=qy(anti_err,j);
    end
    
    
    for j=1:length(centers)
        matfof_err_choice{z,j,1}=qy(contra_err,j);
        matfof_err_choice{z,j,2}=qy(ipsi_err,j);
    end

    waves_fof{z}=CD.wave{ccx};
...
save ../data_files/matfof matfof matfof_err_ap ...
    matfof_err_choice waves_fof centers bin_size
